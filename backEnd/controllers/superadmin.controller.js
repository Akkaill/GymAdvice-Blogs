...
export const getAllUsers = async (req, res) => {
  try {
    ...
    logger.error("Get All Users Error", { message: err.message });
...
export const updateUserRole = async (req, res) => {
  ...
  try {
    ...
    logger.error("Update Role Error", { message: err.message });
    res
      .status(500)
...
export const createAdmin = async (req, res) => {
  ...
  try {
    ...
    await newAdmin.save();
    await createLog(
      "SuperAdmin Created Admin",
      req.user._id,
      `Admin created: ${newAdmin.username}`
    );
    await Notification.create({
      user: newAdmin._id,
    ...
    res
      .status(201)
      .json({ success: true, message: "Admin Created", user: newAdmin });
  } catch (err) {
...
export const revoke = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "User revoked successfully" });
  } catch (err) {
    logger.error("Revoke Error", { message: err.message });
    res
      .status(500)
      .json({ success: false, message: "Server error revoking user" });
  }
};
